...
## Full DRBG leaf evidence

The YASM implementation now includes an independent `bignum_ctr_drbg_update_asm` leaf covering three AES blocks, big-endian `V` increment, provided-data XOR, publication of the 32-byte key and 16-byte `V`, and clearing of its 304-byte private stack area. The C dispatcher selects this leaf only when AES-NI, the independent key expansion, encryption leaf and update leaf are all linked; otherwise it retains the C fallback.

The full entropy/DRBG benchmark performs 10,000 real `getrandom(2)` acquisitions separately, then 512 iterations of the service path. Each iteration includes a periodic OS-backed reseed, 48-byte AdditionalInput through DF/BCC, 2,048 generated bytes with counter handling, state update and zeroization. Five-run medians were **3,068,670,208 ns** for C11 and **16,506,458 ns** for full YASM, giving **185.907x** observed speedup. Entropy acquisition was measured separately because OS scheduling and kernel entropy-path variance should not be conflated with cryptographic leaf throughput.

A test-only assembly build with `BIGNUM_DRBG_ZEROIZE_PROBE` passes a 304-byte post-wipe probe under strict C11 and ASan/UBSan. The production object is assembled without the probe symbol; a production-object string scan found no probe token. The probe is test instrumentation only and is excluded from the controlled production allowlist.
